        ...
        try {
          ...
          if (invoices.length > 0) {
            ...
            if (trialDaysRemaining <= 7) {
              ...
              try {
                const generateResponse = await api.get("/billing/check-trial-invoice");
                ...
                if (generateResponse.data.data) {
                  ...
                  setTimeout(async () => {
                    ...
                    const retryInvoices = retryResponse.data.data.invoices || retryResponse.data.data;
  ...
  const handlePaymentMethodSelect = async (method: "BCA_VA" | "QRIS") => {
    ...
    try {
      ...
      if (pendingInvoice.payment_method_selected === method && isExpired) {
        ...
        // Create new payment
        response = await api.post(`/billing/invoices/${pendingInvoice.id}/pay`, {
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-background">
        {/* ✅ FIXED: Use tripay_payment_url instead of checkout_url */}
        {user.status === "suspended" && <TrialExpiredModal isOpen={true} invoiceUrl={pendingInvoice?.tripay_payment_url} amount={pendingInvoice?.total_amount || user.monthly_bill} dueDate={pendingInvoice?.due_date} />}
