  ...
  return (
    <>
      ...
      <div className="space-y-6">
        {/* Welcome Section */}
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Selamat datang kembali, {user?.business_name || "User"}!</h1>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 dark:text-white mt-2">Rp {(pendingInvoice?.total_amount || user?.monthly_bill || 0).toLocaleString("id-ID")}</p>
                </div>
              ...
              <div className="mt-4 flex items-center text-sm">
                <span className="text-gray-600 dark:text-gray-400">Status: {user?.status === "trial" ? "Trial" : user?.status === "active" ? "Aktif" : user?.status === "suspended" ? "Tersuspend" : user?.status || "N/A"}</span>
              </div>
            </div>
          </div>
        )}
