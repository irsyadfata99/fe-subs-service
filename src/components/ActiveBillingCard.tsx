...
interface ActiveBillingCardProps {
  nextBillingDate: string;
  monthlyBill: number;
}

export default function ActiveBillingCard({
  nextBillingDate,
  monthlyBill,
}: ActiveBillingCardProps) {
  ...
  // Use consolidated invoice hook
  const { invoice: pendingInvoice, loading: isLoadingInvoice } =
    usePendingInvoice();
  ...
  // Hanya tampil jika ada pending invoice atau kurang dari 7 hari ke billing date
  if (isDismissed || (!pendingInvoice && daysLeft > 7)) return null;

  const billingAmount = pendingInvoice?.total_amount || monthlyBill;

  const handlePayNow = () => {
    const isExpired =
      paymentData?.tripay_expired_time &&
      new Date(paymentData.tripay_expired_time).getTime() < Date.now();
  ...
  const buttonLabel = paymentMethod
    ? `Bayar via ${paymentMethod === "BCA_VA" ? "BCA VA" : "QRIS"}`
    : "Pilih Metode Pembayaran";

  // Tentukan warna gradient berdasarkan kondisi
  const gradientClass = pendingInvoice
    ? "from-orange-500 to-orange-600" // Ada invoice pending
    : "from-green-500 to-green-600"; // Tidak ada invoice, reminder saja

  const buttonTextClass = pendingInvoice
    ? "text-orange-600 hover:bg-orange-50" // Orange untuk pending invoice
    : "text-green-600 hover:bg-green-50"; // Green untuk reminder biasa

  const titleText = pendingInvoice
    ? "Invoice Pending - Tagihan Jatuh Tempo"
    : `Tagihan berikutnya dalam ${daysLeft} hari`;

  const descriptionText = pendingInvoice
    ? `Anda memiliki tagihan yang perlu dibayar sebelum ${new Date(
    ...
    : `Tagihan bulanan Anda akan jatuh tempo pada ${new Date(
        nextBillingDate
      ).toLocaleDateString("id-ID", {
        day: "numeric",
        month: "long",
  ...
  return (
    <>
      <div
        className={`bg-gradient-to-r ${gradientClass} text-white rounded-lg shadow-md p-4 relative`}
      >
        <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-bold mb-1">{titleText}</h3>

            <p className="text-white/90 text-sm mb-3">{descriptionText}</p>

            <div className="bg-white/15 rounded-md p-3 mb-3 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <span className="text-white/90 text-sm">
                  {pendingInvoice ? "Tagihan Saat Ini:" : "Tagihan Bulanan:"}
                </span>
            ...
            <button
              ...
              className={`px-5 py-2 bg-white ${buttonTextClass} font-semibold rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm`}
            >
          ...
          <div className="space-y-4 py-4">
            {pendingInvoice && (
              <InvoiceCard
                ...
                totalAmount={pendingInvoice.total_amount}
