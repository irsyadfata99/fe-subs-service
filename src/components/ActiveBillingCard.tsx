...
export default function ActiveBillingCard({ nextBillingDate, monthlyBill, pendingInvoice, isLoadingInvoice = false }: ActiveBillingCardProps) {
  ...
  const titleText = pendingInvoice ? "Invoice Pending - Tagihan Jatuh Tempo" : `Tagihan berikutnya dalam ${daysLeft} hari`;

  const descriptionText = pendingInvoice
    ? `Anda memiliki tagihan yang perlu dibayar sebelum ${new Date(pendingInvoice.due_date).toLocaleDateString("id-ID", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })}. Lakukan pembayaran untuk melanjutkan layanan.`
    : `Tagihan bulanan Anda akan jatuh tempo pada ${new Date(nextBillingDate).toLocaleDateString("id-ID", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })}. Lakukan pembayaran untuk melanjutkan layanan.`;
  ...
  return (
    <>
      <div className={`bg-gradient-to-r ${gradientClass} text-white rounded-lg shadow-md p-4 relative`}>
          ...
          <div className="flex-1">
            ...
            <div className="bg-white/15 rounded-md p-3 mb-3 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <span className="text-white/90 text-sm">{pendingInvoice ? "Tagihan Saat Ini:" : "Tagihan Bulanan:"}</span>
            ...
            <button onClick={handlePayNow} disabled={isLoadingInvoice} className={`px-5 py-2 bg-white ${buttonTextClass} font-semibold rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm`}>
