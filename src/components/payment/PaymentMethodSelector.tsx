...
export default function PaymentMethodSelector({
  ...
  const handleCreatePayment = async () => {
    ...
    try {
      ...
      const isExpired =
        currentPaymentData?.tripay_expired_time &&
        new Date(currentPaymentData.tripay_expired_time).getTime() < Date.now();
      ...
      if (currentMethod === selectedMethod && isExpired) {
        ...
        response = await api.post(
          `/billing/invoices/${invoiceId}/regenerate-payment`
        );
        toast.success("Pembayaran berhasil dibuat ulang!");
      } else {
        // Create new payment
        response = await api.post(
          `/billing/invoices/${invoiceId}/create-payment`,
          {
      ...
      const paymentData: PaymentData = {
        tripay_payment_url:
          ...
          result.payment?.tripay_payment_url,
        tripay_va_number:
          result.invoice?.tripay_va_number || result.payment?.tripay_va_number,
        tripay_qr_url:
          result.invoice?.tripay_qr_url || result.payment?.tripay_qr_url,
        tripay_expired_time:
          result.invoice?.tripay_expired_time ||
          result.payment?.tripay_expired_time,
        tripay_reference:
          result.invoice?.tripay_reference || result.payment?.tripay_reference,
      };
    ...
    <div className="space-y-4">
      ...
      <div className={`flex gap-3 ${isModal ? "pt-2" : ""}`}>
        ...
        <Button
          ...
          className={`gap-2 ${onCancel ? "flex-1" : "w-full"}`}
          size="lg"
        >
          {isCreating ? (
            <>
